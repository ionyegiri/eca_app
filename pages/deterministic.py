# pages/deterministic.py (updated)
...
from utils_ui import (
    upload_or_build_stress_strain, upload_or_define_jr_curve,
    upload_histogram, paste_histogram,
...
def _build_inputs() -> DeterministicInputs:
    ...
    # --- EOL toughness input ---
    st.subheader("EOL critical fracture toughness")
    tough_mode = st.radio("Provide toughness as", ["Kmat (MPa√m)", "Jc (N/mm)"], horizontal=True)
    if tough_mode.startswith("Kmat"):
        Kmat = st.number_input("Kmat (MPa√m)", min_value=1.0, value=200.0, step=1.0)
        Jc = None
    else:
        Jc = st.number_input("Jc (N/mm)", min_value=1.0, value=150.0, step=1.0)
        Kmat = None

    jr = upload_or_define_jr_curve(key_prefix="det_")
    ...
    # Histograms – upload or paste
    st.subheader("Fatigue histograms")
    tab_i, tab_o = st.tabs(["Installation", "Operation"])
    with tab_i:
        hi_mode = st.radio("Source", ["Upload CSV", "Paste CSV"], key="det_hist_i_mode")
        hist_i = upload_histogram("Installation histogram", key_prefix="det_") if hi_mode.startswith("Upload") else paste_histogram("Installation histogram", key_prefix="det_")
    with tab_o:
        ho_mode = st.radio("Source", ["Upload CSV", "Paste CSV"], key="det_hist_o_mode")
        hist_o = upload_histogram("Operation histogram", key_prefix="det_") if ho_mode.startswith("Upload") else paste_histogram("Operation histogram", key_prefix="det_")
